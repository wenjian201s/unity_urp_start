...
using UnityEngine.Rendering.Universal; // URP相机扩展数据，用于在URP中请求深度纹理

[RequireComponent(typeof(MeshFilter), typeof(MeshRenderer))]
public class Water : MonoBehaviour {
    public Shader waterShader; //水面渲染脚本

    public Atmosphere atmosphere; //大气散射脚本

    public int planeLength = 10;
    public int quadRes = 10;

    public enum WaveFunction { //波浪类型菜单
        Sine = 0,
        SteepSine,
        Gerstner
    }; 

    public enum WaveType { //波浪类型结构体
        Directional = 0,
        Circular,
    };
    ...
    private Camera cam;
    [SerializeField] private Camera targetCamera; // URP目标相机，如果不手动指定则自动使用Camera.main
    ...
    void CreateMaterial() {
        if (waterShader == null) waterShader = Shader.Find("Custom/WaterURP"); // URP版本水面Shader，如果面板没有指定则自动查找
    ...
    void SetupURPCamera() {
        cam = targetCamera != null ? targetCamera : Camera.main; //优先使用手动指定的URP相机，否则使用主相机

        if (cam == null) {
            cam = FindObjectOfType<Camera>(); //如果场景中没有MainCamera标签，则尝试查找任意相机
        }

        if (cam == null) {
            Debug.LogWarning("WaterURP: 场景中没有找到可用相机，_CameraInvViewProjection不会被更新。");
            return;
        }

        cam.depthTextureMode |= DepthTextureMode.Depth; //在Built-in中用于请求_CameraDepthTexture；URP中保留此设置作为兼容

        UniversalAdditionalCameraData cameraData = cam.GetComponent<UniversalAdditionalCameraData>(); //获取URP相机扩展组件
        if (cameraData != null) {
            cameraData.requiresDepthTexture = true; //URP中要求相机生成_CameraDepthTexture，水面深度相关效果需要它
        }
    }
    ...
    void OnEnable() {
        ...
        CreateMaterial();
        CreateWaveBuffer();
        SetupURPCamera(); //URP中不再依赖名称为Main Camera的对象，统一初始化目标相机和深度纹理设置
    }
